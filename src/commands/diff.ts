...
const getLocalComponentPath = (configPath: string, componentName: string) => {
  ...
  return isTypescript()
    ? path.join(config.ui, `${componentName}.tsx`)
    : path.join(config.ui, `${componentName}.jsx`);
};
...
export const diff = async (...args: string[]) => {
  try {
    ...
    for (const componentName of componentNames) {
      const localComponentPath = getLocalComponentPath(
        configPath,
        componentName,
      );
    ...
    if (changedComponents.length > 0) {
      ...
      if (
        selectedComponents.includes('none') ||
      ...
      for (const componentName of selectedComponents) {
        try {
          ...
          console.error(
            `Error updating ${componentName}: ${(error as Error).message}`,
          );
        }
      }
    } else {
    ...
    console.error('Error checking differences:', (error as Error).message);
  }
};
